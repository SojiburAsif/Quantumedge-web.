...
const PublicServices = () => {
    ...
    const badgeColor = () => ({ bgDot: "bg-green-600", bgPill: "bg-green-50 text-green-600" });
    ...
    const handleBook = async () => {
        ...
        try {
            ...
            Swal.fire({ icon: "error", title: "Booking failed", text: err.response?.data?.message || err.message || "Unable to create booking. See console.", confirmButtonColor: "#dc2626" });
        }
    };
    ...
    if (error) return (
        <div className="p-8 bg-gray-50 min-h-screen flex items-center justify-center">
            <div className="text-center">
                ...
                <button onClick={() => window.location.reload()} className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg shadow transition">Retry</button>
            </div>
        </div>
    );
    ...
    ${featured ? "-mt-8 scale-105 z-30 shadow-2xl" : "hover:-translate-y-2 hover:shadow-lg"}`}
                            ...
                            >
                                {featured && (
                                    <div
                                        ...
                                        style={{
                                            filter: "blur(28px)",
                                            background:
                                                "radial-gradient(closest-side, rgba(34,197,94,0.12), rgba(34,197,94,0.03) 40%, rgba(0,0,0,0) 70%)",
                                        }}
                                    />
                                )}
                                ...
                                <p
                                    className="text-sm text-gray-600 mt-4 mb-4"
                                    style={{
                                        WebkitLineClamp: 3,
                                        display: "-webkit-box",
                                ...
                                <div className="flex flex-wrap items-center gap-2 mb-4">
                                    {shownBadges.length === 0 ? (
                                        ...
                                        shownBadges.map((b, i) => {
                                            const col = badgeColor(b, i);
                                            return (
                                                <span key={i} className={`flex items-center gap-2 text-[11px] px-2 py-1 rounded-full whitespace-nowrap ${col.bgPill}`}>
                                                    <span className={`inline-block w-2 h-2 rounded-full ${col.bgDot}`} />
                                                    {b}
                                                </span>
                                            );
                                        })
                                ...
                                <div className="border-t border-gray-200 pt-4 flex items-center justify-between gap-4">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <button
                                            ...
                                            className="px-4 py-2 rounded-full text-sm font-medium transition bg-green-500 hover:bg-green-600 text-white shadow-md"
                                        >
                                            Apply Now
                                        </button>
                                    </div>
                        ...
                        <div className="p-6">
                            ...
                            <div className="mt-6">
                                ...
                                <div className="mt-4 flex items-center justify-end gap-3">
                                    ...
                                    <button onClick={handleBook} disabled={bookingLoading} className="px-4 py-2 bg-green-500 hover:bg-green-600 text-white rounded inline-flex items-center gap-2 disabled:opacity-60">
